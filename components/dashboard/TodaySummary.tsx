...
export default function TodaySummary({ journals }: Props) {
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs text-slate-500">오늘의 거래</div>
            </div>
          </div>
          <span className="text-xs text-slate-600">아직 거래가 없습니다</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={`bg-slate-900 border rounded-xl p-4 sm:p-5 ${
      isPositive ? 'border-emerald-800/50' : isNegative ? 'border-red-800/50' : 'border-slate-800'
    }`}>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        {/* Left: Date + PnL */}
        <div className="flex items-center gap-4">
          <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
            isPositive ? 'bg-emerald-900/30' : isNegative ? 'bg-red-900/30' : 'bg-slate-800'
          }`}>
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-slate-500">{stats.count}건 거래</span>
            </div>
        ...
        <div className="flex items-center gap-6 text-sm">
          <div className="flex items-center gap-3">
            <div className="text-center">
              <div className="text-xs text-slate-500 mb-0.5">이익</div>
              <span className="font-bold text-emerald-400 tabular-nums">{stats.wins}</span>
            </div>
            <div className="text-center">
              <div className="text-xs text-slate-500 mb-0.5">손실</div>
          ...
          {stats.count > 1 && (
            <>
              ...
              <div className="flex items-center gap-3">
                <div className="text-center">
                  <div className="text-xs text-slate-500 mb-0.5">Best</div>
                ...
                <div className="text-center">
                  <div className="text-xs text-slate-500 mb-0.5">Worst</div>
